...
void Creature::eatenBy(Creature* enemy) {
...
void Creature::update() {
  ...
  // dont update killed particles
  if (isDead()) {
    return;
  }
  
  // after some time after the kill, remove the particle from play
  if (wasKilled) {
    float elapsedSinceKill = ofGetElapsedTimef() - timeKilled;
    if (elapsedSinceKill > (wasLastOfKind ? 2 : 1) * TIME_FOR_CREATURE_RIPPLE) {
      kill();
    }
    return;
  }

  // if we're 0 sized, die!
  if (released && size <= 0) {
    eatenBy(this);
    return;
  }
  ...
  // set the size/mass of the creature
  ...
  // if the creature is still in the creature creator
  if (!released) {
  ...
  creatureMass = MAX(0, size * CREATURE_SIZE_SCALAR + CREATURE_SIZE_MIN);
...
void Creature::draw(float opacity = 1.0) {
  ...
  // if killed, draw a ripple
  if (wasKilled) {
    
    if (!wasLastOfKind) {
    
      float elapsedSinceKill = (ofGetElapsedTimef() - timeKilled) / TIME_FOR_CREATURE_RIPPLE;
      elapsedSinceKill = CLAMP(elapsedSinceKill, 0, 1);
      
      glPushMatrix();
      glTranslatef(x, y, 0);
      glScalef(1 + SIZE_OF_CREATURE_RIPPLE * SQ(elapsedSinceKill),
               1 + SIZE_OF_CREATURE_RIPPLE * SQ(elapsedSinceKill), 1);
      glEnable(GL_BLEND);
      glBlendFunc(GL_SRC_ALPHA, GL_ONE);
      glColor4f(player->color.r, player->color.g, player->color.b, 1-sqrt(elapsedSinceKill));
      ...
      for (int i = 0; i < 3; i++) {
        float elapsedSinceKill =
          (ofGetElapsedTimef() - timeKilled - TIME_FOR_CREATURE_RIPPLE * i * 0.2)
            / TIME_FOR_CREATURE_RIPPLE;
        elapsedSinceKill = CLAMP(elapsedSinceKill, 0, 1);
